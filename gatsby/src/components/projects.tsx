...
const ProjectInfo: FC<ProjectProps> = ({
	...
	<a className='block mb-12 text-left hover:opacity-80 transition-opacity' href={link ?? '#'}>
...
const ProjectYearWrapper: FC<ProjectYearWrapperProps> = ({ yearValue, children }) => {
...
const Projects: FC = () => {
	...
	return years
		.map((year, yIdx) => {
			const projects = (year.projects as ProjectDataWrapped[]).map((proj, pIdx) => (
				<ProjectInfo project={proj.node} key={pIdx} />
			))
