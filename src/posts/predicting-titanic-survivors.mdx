...
description: 'Learn how to build a machine learning model to predict Titanic survivors based on the Kaggle dataset.'
date: 2018-08-04T23:00:00Z
displayDate: 'August 4, 2018'
draft: false
hasMath: true
---
...
![Titanic Dataset Data](./images/titanic-training.png)
...
Next, I converted all of the textual features into numerical values roughly within the range <InlineMath math="(-1, 1)" />:
...
I then normalized the data using a simple approach: <InlineMath math="(\text{value} - \text{max})/\text{range}" />,
where <InlineMath math="\text{range} = \text{max} - \text{min}"/>.
...
Add an <InlineMath math="x_0 = 1"/> column to the features array so everything looks clean:
...
Using the features and labels arrays, create a _design matrix_ <InlineMath math="X"/> and the _label vector_ <InlineMath math="y"/>:
...
Using <InlineMath math="X"/> and <InlineMath math="y"/>:, it's time to define some variables that are important to the implmentation of the _cost function_ <InlineMath math="J"/>:
...
Let's define the parameter <InlineMath math="\theta"/> as a <InlineMath math="n+1"/>-dimensional vector, initially filled with zeros:

```python
theta = np.asmatrix(np.zeros(n + 1)).T
```

And let's set a learning rate <InlineMath math="\alpha"/>, in this case I just chose an arbitrary value:

```python
alpha = 0.001
```

It's important to choose the right <InlineMath math="\alpha"/>, because a smaller value may make the model slower to converge, while a larger value might make the model never converge.
...
To generate a matrix of predictions using the parameter vector <InlineMath math="\theta" />, we define our hypothesis function <InlineMath math="h_{\theta}(x)" /> as follows:

<BlockMath math="h_{\theta}(x) = g(X\theta)" />

where <InlineMath math="X"/> is the design matrix.

But what does the output of this function mean? <InlineMath math="h_{\theta}(x)" /> is actually outputting the _probability_ that the output is 1.
Formally, this function gives you the probablility that <InlineMath math="y=1" />, given <InlineMath math="x" />, parameterized by <InlineMath math="\theta" /> (mathematically, <InlineMath math="P(y=1|x;\theta)" />).
...
<BlockMath math="J(\theta) = \frac{1}{m} \bigg(-y^T \log{h(x)}- (1-y^T)\log{(1-h(x))}\bigg)" />
...
The last (and probably most important) piece of this machine learning algorithm is the `gradient_descent_step` function, which calculates the gradient for <InlineMath math="\theta" />.

The update rule for <InlineMath math="\theta" /> is as follows:

<BlockMath math="\theta := \theta - \frac{\alpha}{m}X^T(g(X\theta)-\vec{y})" />
...
Before we do anything else, let's save the optimal <InlineMath math="\theta" /> in a file:
...
Using the `hypothesis(x)` function, let's define a function that outputs `1` if <InlineMath math="P(y=1|x;\theta) ≥ 0.5" /> or `0` otherwise:
...
![Kaggle leaderboard screenshot](./images/kaggle-score-logistic.png)
